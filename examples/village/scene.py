...
from meshcat.geometry import Box, Sphere, Cylinder, MeshLambertMaterial, TriangularMeshGeometry,tf
...
from pydrake.all import VPolytope, Hyperrectangle
...
class Village(EnvironmentVisualizer):
    ...
    def building(self, name, c, r, n):
        ...
        for i in range(n[0]):
            for j in range(n[2]):
                ...
                lij = cij - dw
                uij = cij + dw
                windows.append(self.box_visual(name + f'window/_window_x_{i}_{j}', lij, uij, wcolor, self.opacity))
                lij -= df
                uij += df
                windows.append(self.box_visual(name + f'window/_frame_x_{i}_{j}', lij, uij, fcolor, self.opacity))
        ...
        for i in range(n[1]):
            for j in range(n[2]):
                ...
                lij = cij - dw
                uij = cij + dw
                windows.append(self.box_visual(name + f'window/_window_y_{i}_{j}', lij, uij, wcolor, self.opacity))

                lij -= df
                uij += df
                windows.append(self.box_visual(name + f'window/_frame_y_{i}_{j}', lij, uij, fcolor, self.opacity))
        
        return body, roof, windows
    
    def build(self, village_height = 5, village_side = 19, building_every = 5, density = 0.3, opacity = 1, seed = 12, r_uav = 0.1):
        np.random.seed(seed)
        ...
        def _tree(i, j):
            ...
            r = .5 - r_uav # Radius of foliage (shrunk by UAV radius).
            r_trunk = .1
            low = [i + .5, j + .5, 1]
            high = [i + .5, j + .5, village_height - .5]
            c = np.random.uniform(low, high)
            self.tree(name, c, r, r_trunk)
            Lij, Uij = outer_box(i, j, c[0], c[1], r_trunk + r_uav, 0, c[2] - .5) # Around trunk.
            Lij.append([i,     j,     c[2] + .5])  # Above foliage.
    ...
    def plot_plane(self, name, a, b, bounds=None, color=(0.5, 0.5, 1.0), opacity=0.5, thickness=0.01):
        """
...
def plot_edge(world, pt1, pt2, name, color, opacity, size = 0.01):
    ...
    world['/'+name].set_object(
                        Cylinder(np.float64(np.linalg.norm(pt1-pt2)), size),
                        material)
...
def plot_points(world, points, name, color, opacity, size = 1):
    for idx, pt in enumerate(points):
        world[f'/{name}/{idx}'].set_object(Sphere(size), MeshLambertMaterial(color = to_hex((color[0],color[1],color[2])), opacity = opacity))
...
def plot_composite_bezier_curve(env, traj: CompositeBezierCurve, name, color = [0,0,0], opacity = 1, size= 0.05):
