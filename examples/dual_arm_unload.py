...
``MinimumTimePlanner``, default) or ``scs`` (the ``SCSPlanner`` baseline). The
...
BRICK_CENTROIDS = np.array([
    [PALLET_CENTER[0] + dx, PALLET_CENTER[1] + dy, BRICK_CENTROID_Z]
    for dx, dy in BRICK_OFFSETS
])
...
BRICK_GRASP_POINTS = np.array([
    [PALLET_CENTER[0] + dx, PALLET_CENTER[1] + dy, TRAJ_GRASP_Z]
    for dx, dy in BRICK_OFFSETS
])
...
ARM_RED_PARTS = [
    ...
    ('finger_L', np.array([0.040012, 0.030016, 0.120000]), np.array([-0.060000, -0.000024, -0.019000])),
    ('finger_R', np.array([0.040012, 0.030016, 0.120000]), np.array([0.060000, 0.000024, -0.019000])),
]
ARM_BLUE_PARTS = [
    ('hand', np.array([0.240080, 0.200096, 0.280000]), np.array([0.000000, 0.000000, 0.200000])),
    ('finger_L', np.array([0.040012, 0.030016, 0.120000]), np.array([0.060000, 0.000024, -0.019000])),
...
BASES = [("base_blue", BASE_BLUE[0], BASE_BLUE[1]),
         ("base_red", BASE_RED[0], BASE_RED[1])]
...
def obstacle_box_vs_fixed(moving_dims, fixed_dims, fixed_center, robot_idx,
...
def phase_obstacles(is_held, stationary_bricks):
    ...
    if is_held:
        ...
        for arm_idx in (0, 1):
            for b_name, b_dims, b_center in BASES:
                obstacles.append(obstacle_box_vs_fixed(BRICK_DIMS, b_dims, b_center, arm_idx, brick_offset))
                descs.append(f"arm{arm_idx}_held_brick_vs_{b_name}")

        # held brick vs walls
        for arm_idx in (0, 1):
            for w_name, w_dims, w_center in WALLS:
                obstacles.append(obstacle_box_vs_fixed(BRICK_DIMS, w_dims, w_center, arm_idx, brick_offset))
                descs.append(f"arm{arm_idx}_held_brick_vs_{w_name}")

        # held brick vs stationary bricks
        for arm_idx in (0, 1):
            for s_name, s_dims, s_center in stationary_bricks:
                obstacles.append(obstacle_box_vs_fixed(BRICK_DIMS, s_dims, s_center, arm_idx, brick_offset))
...
def generate_approach_motion(start_blue, start_red, goal_blue, goal_red,
                             stationary_packages, gripper_dims,
                             pallet_center, pallet_dims,
                             finger_below_tcp=FINGER_BELOW_TCP, clearance=0.05,
    ...
    waypoints.append(_add_to_last(
        waypoints, np.array([0, 0, safe_z - waypoints[-1][2]]),
    ...
    waypoints.append(_add_to_last(
        waypoints, np.array([red_lane_x - waypoints[-1][0], 0, 0]),
        np.array([blue_lane_x - waypoints[-1][3], 0, 0])))

    waypoints.append(_add_to_last(
        waypoints, np.array([0, goal_red[1] - waypoints[-1][1], 0]),
    ...
    if abs(delta_red_x) > 1e-3 or abs(delta_blue_x) > 1e-3:
        waypoints.append(_add_to_last(
            waypoints, np.array([delta_red_x, 0, 0]),
            np.array([delta_blue_x, 0, 0])))

    waypoints.append(_add_to_last(
        waypoints, goal_red - waypoints[-1][:3], goal_blue - waypoints[-1][3:]))
    return np.array(waypoints)


def generate_forward_motion(start_blue, start_red, goal_blue, goal_red,
                            blue_brick_dims, red_brick_dims,
                            stationary_brick_dims, gripper_dims,
                            pallet_center, pallet_dims,
                            offload_blue_center, offload_red_center):
    ...
    waypoints.append(_add_to_last(
        waypoints, np.array([0, 0, required_lift_height]),
        np.array([0, 0, required_lift_height])))

    x_clearance = 0.025
    pallet_width = pallet_dims[0]
    blue_lane_x = pallet_center[0] - pallet_width / 2 + np.max(
        [gripper_dims[0], blue_brick_dims[0]]) / 2 + x_clearance
    red_lane_x = pallet_center[0] + pallet_width / 2 - np.max(
        [gripper_dims[0], red_brick_dims[0]]) / 2 - x_clearance
    ...
    y_thresh = np.max([gripper_dims[1],
                       blue_brick_dims[1] / 2 + red_brick_dims[1] / 2])
    if y_separation < y_thresh + 0.02 and arms_crossed:
        ...
        waypoints.append(_add_to_last(
            waypoints, clearing_delta[:3], clearing_delta[3:]))

    waypoints.append(_add_to_last(
        waypoints, np.array([delta_x_red_ferry, 0, 0]),
        np.array([delta_x_blue_ferry, 0, 0])))

    target_y_blue = offload_blue_center[1] + OFFLOAD_DIMS[1] / 2 - np.max(
        [gripper_dims[1], blue_brick_dims[1]]) / 2 - 0.02
    target_y_red = offload_red_center[1] + OFFLOAD_DIMS[1] / 2 - np.max(
        [gripper_dims[1], red_brick_dims[1]]) / 2 - 0.02
    waypoints.append(_add_to_last(
        waypoints, np.array([0, target_y_red - waypoints[-1][1], 0]),
        np.array([0, target_y_blue - waypoints[-1][4], 0])))

    delta_xy_red = goal_red[:2] - waypoints[-1][:2]
    delta_xy_blue = goal_blue[:2] - waypoints[-1][3:5]
    waypoints.append(_add_to_last(
        waypoints, np.array([delta_xy_red[0], delta_xy_red[1], 0]),
...
def _walls_for_zone(zone_name, brick_aabb):
    ...
    return [
        (f"wall_{zone_name}_N", np.array([len_ns_x, WALL_THICK, WALL_HEIGHT]),
         np.array([(wx_lo + wx_hi) / 2, wy_hi, wz])),
        (f"wall_{zone_name}_S", np.array([len_ns_x, WALL_THICK, WALL_HEIGHT]),
         np.array([(wx_lo + wx_hi) / 2, wy_lo, wz])),
        (f"wall_{zone_name}_E", np.array([WALL_THICK, len_ew_y, WALL_HEIGHT]),
         np.array([wx_hi, (wy_lo + wy_hi) / 2, wz])),
        (f"wall_{zone_name}_W", np.array([WALL_THICK, len_ew_y, WALL_HEIGHT]),
...
def build_all_walls():
    ...
    grey_aabb = _brick_aabb([(OFFLOAD_GREY[0] - SLOT_OFFSET_X, OFFLOAD_GREY[1]),
                             (OFFLOAD_GREY[0] + SLOT_OFFSET_X, OFFLOAD_GREY[1])])
    red_aabb = _brick_aabb([(OFFLOAD_RED[0] - SLOT_OFFSET_X, OFFLOAD_RED[1]),
                            (OFFLOAD_RED[0] + SLOT_OFFSET_X, OFFLOAD_RED[1])])
    return (_walls_for_zone("pallet", pallet_aabb)
...
def plan_phase(name, waypoints, is_held, stationary_bricks, planner, vel_set, acc_set):
    ...
    free = hits == {}
    print(f"  T={res.total_time:.2f}s ({extra}), solve {solve_s:.2f}s, "
          f"collision-free={free}")
    if not free:
        for seg_id, obs_ids in hits.items():
            for oid in obs_ids:
                print(f"    seg {seg_id} hits obs {oid}: "
...
def render(path_out, phases, planner, total_T):
    ...
    for _, _, w_dims, w_center in [("", *w) for w in WALLS]:
        ...
        ax_top.add_patch(mpatches.Rectangle(
            lb, w_dims[0], w_dims[1], facecolor="0.4", edgecolor="none",
            alpha=0.6, zorder=2))
    for c in BRICK_CENTROIDS:
        ax_top.add_patch(mpatches.Rectangle(
            c[:2] - BRICK_DIMS[:2] / 2, BRICK_DIMS[0], BRICK_DIMS[1],
            facecolor="tan", edgecolor="k", alpha=0.7, zorder=3))
    for bid in range(4):
        p = get_drop_pos(bid)
        col = "0.6" if BRICK_COLOR[bid] == "grey" else "lightcoral"
        ax_top.add_patch(mpatches.Rectangle(
            p[:2] - BRICK_DIMS[:2] / 2, BRICK_DIMS[0], BRICK_DIMS[1],
            facecolor=col, edgecolor="k", alpha=0.7, zorder=3))
    ...
    fig.suptitle(f"Dual-arm full unload ({planner.upper()}): "
...
def animate(meshcat, phases, held_assignment):
    ...
    brick_rgba = {bid: (pd.Rgba(0.55, 0.55, 0.55, 1.0) if BRICK_COLOR[bid] == "grey"
                        else pd.Rgba(0.85, 0.10, 0.10, 1.0)) for bid in range(4)}
    for bid in range(4):
    ...
    for pi, (name, traj, T, _) in enumerate(phases):
        ...
        for t in ts:
            ...
            for bid in range(4):
                ...
                meshcat.SetTransform(f"scene/brick_{bid}",
                                     pd.RigidTransform(pos),
                                     time_in_recording=t_global)
        for arm, hid in held.items():
...
def main(planner="bcp", show_meshcat=True):
    ...
    stat_AB = named_bricks([(M2_BLUE, BRICK_CENTROIDS[M2_BLUE]),
                            (M2_RED, BRICK_CENTROIDS[M2_RED])])
    stat_C = named_bricks([(M2_BLUE, BRICK_CENTROIDS[M2_BLUE]),
                           (M2_RED, BRICK_CENTROIDS[M2_RED]),
                           (M1_BLUE, drop_centroid(M1_BLUE)),
                           (M1_RED, drop_centroid(M1_RED))])
    stat_D = named_bricks([(M1_BLUE, drop_centroid(M1_BLUE)),
                           (M1_RED, drop_centroid(M1_RED))])
    ...
    wp_A = generate_approach_motion(
        start_blue=P_BLUE_QREF, start_red=P_RED_QREF,
        goal_blue=BRICK_GRASP_POINTS[M1_BLUE], goal_red=BRICK_GRASP_POINTS[M1_RED],
        stationary_packages=[(BRICK_CENTROIDS[M2_BLUE], BRICK_DIMS),
                             (BRICK_CENTROIDS[M2_RED], BRICK_DIMS)],
        gripper_dims=GRIPPER_DIMS_PLANNER,
        pallet_center=PALLET_CENTER, pallet_dims=PALLET_DIMS)

    # Phase B: grasp move-1 -> offload (held)
    wp_B = generate_forward_motion(
        start_blue=BRICK_GRASP_POINTS[M1_BLUE], start_red=BRICK_GRASP_POINTS[M1_RED],
        goal_blue=get_drop_pos(M1_BLUE), goal_red=get_drop_pos(M1_RED),
        blue_brick_dims=BRICK_DIMS, red_brick_dims=BRICK_DIMS,
    ...
    wp_C = generate_approach_motion(
        start_blue=get_drop_pos(M1_BLUE), start_red=get_drop_pos(M1_RED),
        goal_blue=BRICK_GRASP_POINTS[M2_BLUE], goal_red=BRICK_GRASP_POINTS[M2_RED],
        stationary_packages=[(BRICK_CENTROIDS[M2_BLUE], BRICK_DIMS),
                             (BRICK_CENTROIDS[M2_RED], BRICK_DIMS),
    ...
    wp_D = generate_forward_motion(
        start_blue=BRICK_GRASP_POINTS[M2_BLUE], start_red=BRICK_GRASP_POINTS[M2_RED],
        goal_blue=get_drop_pos(M2_BLUE), goal_red=get_drop_pos(M2_RED),
        blue_brick_dims=BRICK_DIMS, red_brick_dims=BRICK_DIMS,
        stationary_brick_dims=[BRICK_DIMS, BRICK_DIMS],
        gripper_dims=GRIPPER_DIMS_PLANNER,
        pallet_center=PALLET_CENTER, pallet_dims=PALLET_DIMS,
    ...
    wp_E = generate_approach_motion(
        start_blue=get_drop_pos(M2_BLUE), start_red=get_drop_pos(M2_RED),
        goal_blue=P_BLUE_QREF, goal_red=P_RED_QREF,
    ...
    print(f"\n=== Total: T = {total_T:.2f}s, solve = {total_solve:.2f}s "
          f"({planner.upper()}) ===")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--no-meshcat", action="store_true",
                        help="skip the meshcat animation (run headless)")
