...
def _as_float64_curve(traj: pb.CompositeBezierCurve) -> pb.CompositeBezierCurve:
    ...
    return pb.CompositeBezierCurve([
        pb.BezierCurve(np.asarray(c.points, dtype=float),
                       c.initial_time, c.final_time)
...
def _biconvex_with_pruning(
    ...
    surviving = [(i, r) for i, (r, d) in enumerate(zip(regions, dists))
    ...
    if verbose and len(pruned) != len(regions):
        print(f"[pybmt-scs] pruned {len(regions) - len(pruned)} near-zero "
    ...
    trajectory = biconvex(
        q_init, q_term, pruned, velocity_set, acceleration_set,
        trajectory_degree, tol=rel_term)
    return trajectory, pruned


class SCSPlanner:
    """Minimum-time Bezier planner via convex regions + ``scsplanning``.
    ...
    def solve(
        ...
        if not isinstance(domain, pd.HPolyhedron):
            raise ValueError(
                f"domain must be an HPolyhedron, got {type(domain).__name__}.")
        ...
        t0 = time.time()
        regions = construct_regions_from_obstacles(
            path, obstacles, domain, margin=self.margin)
        ...
        if self.solver_type == "biconvex":
            trajectory, solved_regions = _biconvex_with_pruning(
                q_init, q_term, regions, velocity_set, acceleration_set,
                self.trajectory_degree, path, self.rel_term,
                self.split_margin, self.min_edge_length, verbose)
...
def solve_scs_minimum_time(
    ...
    **planner_kwargs,
    ...
    """One-shot convenience wrapper around :class:`SCSPlanner`.

    Extra keyword arguments are forwarded to the :class:`SCSPlanner`
    constructor (e.g. ``trajectory_degree``, ``solver_type``, ``rel_term``).
    """
    return SCSPlanner(**planner_kwargs).solve(
